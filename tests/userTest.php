 ...
 */
class userCase extends TestCase {
  ...
  public function testUser() {
    ...
    $this->brush('user-create', array($name), $options + array('password' => 'password', 'mail' => "example@example.com"));
    ...
    // user-block
    $this->brush('user-block', array($name), $options);
    $this->brush('user-information', array($name), $options + array('pipe' => NULL));
    $output = $this->getOutput();
    $row  = str_getcsv($output);
    ...
    // user-unblock
    $this->brush('user-unblock', array($name), $options);
    $this->brush('user-information', array($name), $options + array('pipe' => NULL));
    $output = $this->getOutput();
    $row  = str_getcsv($output);
    ...
    $this->brush('user-add-role', array('administrator', $name), $options);
    $this->brush('user-information', array($name), $options + array('pipe' => NULL));
    $output = $this->getOutput();
    $row  = str_getcsv($output);
    ...
    // user-remove-role
    $this->brush('user-remove-role', array('administrator', $name), $options);
    $this->brush('user-information', array($name), $options + array('pipe' => NULL));
    $output = $this->getOutput();
    $row  = str_getcsv($output);
    ...
    // user-password
    $newpass = 'newpass';
    $this->brush('user-password', array($name), $options + array('password' => $newpass));
    ...
    // user-login
    $this->brush('user-login', array($name), $options);
    $output = $this->getOutput();
    ...
    ";
    $this->brush('php-eval', array($eval), $options);
    $this->brush('user-cancel', array($name), $options + array('delete-content' => NULL));
