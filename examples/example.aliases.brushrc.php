 ...
 *
 *   $ brush @dev status
 *   $ brush --root=/path/to/backdrop --uri=dev.mybackdropsite.com status
 *
 * See the --uri option documentation below for hints on setting its value.
 *
 * Any option that can be placed on the brush commandline
 ...
 *      locations:
 *        a. /etc/brush
 *        b. In the brush installation folder
 *        c. Inside the 'aliases' folder in the brush installation folder
 *        d. $HOME/.brush
 ...
 * "brush rsync" and "brush sql-sync".
 *
 ...
 *                      aliases in the group (e.g. brush @mybackdropsite status)
 *
 ...
 *
 *   $ brush site-alias @self
 *
 * If you would like to see all of the Backdrop sites at a specified
 * root directory, use the built-in alias "@sites":
 *
 *   $ brush -r /path/to/backdrop site-alias @sites
 *
 * The built-in alias "@none" represents the state of no Backdrop site;
 * to ignore the site at the cwd and just see default brush status:
 *
 *   $ brush @none status
 *
 * See `brush help site-alias` for more options for displaying site
 * aliases.  See `brush topic docs-bastion` for instructions on configuring
 ...
 *     If 'db-url' is not provided, then brush will automatically look it
 ...
 *     record.  Brush will look up the 'databases' record if it is not specified.
 * - 'path-aliases': An array of aliases for common rsync targets.
 *   Relative aliases are always taken from the Backdrop root.
 *     '%brush-script': The path to the 'brush' script, or to 'brush.php' or
 *       'brush.bat', as desired.  This is used by backend invoke when brush
 *       runs a brush command.  The default is 'brush' on remote machines, or
 *       the full path to brush.php on the local machine.
 *     '%brush': A read-only property: points to the folder that the brush script
 *       is stored in.
 *     '%dump-dir': Path to directory that "brush sql-sync" should use to store
 *       sql-dump files. Helpful filenames are auto-generated.
 *     '%dump': Path to the file that "brush sql-sync" should use to store sql-dump file.
 ...
 *      brush @stage sql-sync @self @live
 *      brush sql-sync @stage @live
 *      brush sql-sync @live @stage
...
#    'path-aliases' => array(
#      '%brush' => '/path/to/brush',
#      '%brush-script' => '/path/to/brush/brush',
