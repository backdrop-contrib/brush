 ...
 * 4. User's .drop folder (i.e. ~/.drop/droprc.php).
 * 5. System wide configuration folder (e.g. /etc/drop/droprc.php).
 * 6. Drop installation folder.
 *
 ...
 * particular version of drop, then you may place them in a file
 ...
 *
 * At its core, drop works by "bootstrapping" the Backdrop environment
 * in very much the same way that is done during a normal page request
 * from the web server, so most drop commands run in the context
 ...
 * for remote sites.
 *
 * See `drop topic docs-bootstrap` for more information on how
 * bootstrapping affects the loading of drop configuration files.
 */
...
// You should not use drop-4.x on Windows; upgrade to the 5.x branch.
...
// Control automatically check for updates in pm-updatecode and drop version.
// FALSE = never check for updates.  'head' = allow updates to drop-HEAD.
// TRUE (default) = allow updates to latest stable release.
# $options['self-update'] = FALSE;

// By default, drop will download projects compatibile with the
...
// Specify additional directories to search for *.drop.inc files
// Separate by : (Unix-based systems) or ; (Windows).
# $options['i'] = 'sites/default:profiles/myprofile';

// Specify modules to ignore when searching for *.drop.inc files
...
// if doing drop development.  Also make sure that error_reporting is set to E_ALL
// in your php configuration file.  See 'drop status' for the path to your php.ini file.
...
// drop).  Note that drop requires at least rsync version 2.6.4
...
// See: http://backdrop.org/node/955092
# $options['rsync-version'] = '2.6.9';

/*
* The output charset suitable to pass to iconv PHP function as out_charset
* parameter. Drop will convert its output from UTF-8 to the charset specified
 ...
 */
// By default, drop will prepend the name of the
...
// List of drop commands or aliases that should override built-in
// shell functions and commands; otherwise, built-ins override drop
// commands. Default is help,dd,sa.
// Warning:  bad things can happen if you put the wrong thing here
// (e.g. eval, grep), so be cautious.
// If a drop command overrides a built-in command (e.g. bash help),
// then you can use the `builtin` operator to run the built-in version
// (e.g. `builtin help` to show bash help instead of drop help.)
// If a drop command overrides a shell command (e.g. grep), then
...
// Load a droprc.php file from the 'drop' folder at the root
...
#if (!empty($output)) {
#  $repo = $output[0];
#  $options['config'] = $repo . '/../drop/droprc.php';
#  $options['include'] = $repo . '/../drop';
#}
